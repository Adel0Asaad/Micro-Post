...
export default function FollowingFeedPage() {
  ...
  const fetchPosts = useCallback(async () => {
    try {
      const postsData = await apiFetch<PostsResponse>(
        '/next-proxy/follows/following/posts',
      );
      setPosts(postsData.body?.posts || []);
    } catch {
      setError(t('followingFeed.error'));
    } finally {
      setIsLoading(false);
    }
  }, [t]);
  ...
  const handleCreatePost = async (content: string) => {
    ...
    if (response.headers.status >= 400) {
      throw new Error(
        (response.body as ApiErrorBody)?.error ||
          t('posts.create.error.failed'),
      );
    }
  ...
  return (
    <PageContainer
      title={t('followingFeed.title')}
      subtitle={t('followingFeed.subtitle')}
    >
      {error && (
